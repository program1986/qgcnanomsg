...
int main() {
    ...
    int bind_result = nn_bind(sock, "tcp://127.0.0.1:5556");
    if (bind_result < 0) {
    ...
    char buf[1024] = {0};
    while (true) {
        std::cout << "Enter a message to send to Endpoint 1: ";
        std::cin.getline(buf, sizeof(buf));

        int bytes = nn_send(sock, buf, strlen(buf), NN_DONTWAIT);
        if (bytes > 0) {
            std::cout << "Endpoint 2 sent message: " << buf << std::endl;
        }

        int recv_bytes = nn_recv(sock, buf, sizeof(buf), NN_DONTWAIT);
        if (recv_bytes > 0) {
            std::cout << "Endpoint 2 received message: " << buf << std::endl;
        }
    }
